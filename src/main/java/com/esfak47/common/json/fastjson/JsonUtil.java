...
/**
 * {@code JsonUtil}类是基于<a href="https://github.com/alibaba/fastjson" style="color:red;font-size:14px;">fastjson</a
 * >进行一层封装，提供Java对象和json字符串之间的相互转换。
 * 由于fastjson较Jackson，gson等解析JSON的jar出现较晚，吸收了实际应用中的一些特点， 所以可以认为fastjson相对效率较高。
 * <p>请注意：本工具类提供的方法默认禁用了循环引用的问题，就不会出现$ref等字符。所谓循环引用一般出现在引用同一个对象（对象重复），对象自引用（父子关系都是一个类）
 * 没有禁用循环引用时的json字符串为：[{"group":{"groupName":"Test组","id":1},"hobbies":["游泳","上网"],"id":1,"password":"123456"},
 * {"group":{"groupName":"Test组","id":2},"hobbies":["电影","book"],"id":2,"password":"123456"},{"birthDay":"1992-02-12
 * 15:32:09","group":{"$ref":"$[1].group"},"hobbies":["电影","book"],"id":3,"password":"xxxxxx"}]
 * 禁用了循环引用后的json字符串为：[{"group":{"groupName":"Test组","id":1},"hobbies":["游泳","上网"],"id":1,"password":"123456"},
 * {"group":{"groupName":"Test组","id":2},"hobbies":["电影","book"],"id":2,"password":"123456"},{"birthDay":"1992-02-12
 * 15:31:28","group":{"groupName":"Test组","id":2},"hobbies":["电影","book"],"id":3,"password":"xxxxxx"}] </p>
...
public class JsonUtil {
    ...
    /**
     * 将Java对象转换JSON字符串. <p> 中文字符换转成unicode码;如果不希望转换unicode码,则使用{@link #toJson(Object, boolean, String...)} </p>
     * <pre>
     ...
     * 如果想要针对某个类的属性进行过滤请使用{@link #toJson(Object, Map)} <p> 中文字符换转成unicode码;如果不希望转换unicode码,则使用{@link #toJson(Object,
     * boolean, String...)} </p>
     * <pre>
     *    User user = new User("admin","123456");
     ...
     * <li>{@link com.alibaba.fastjson.serializer.SerializerFeature#UseSingleQuotes} 使用单引号，默认使用双引号</li> <li>{@link
     * com.alibaba.fastjson.serializer.SerializerFeature#WriteDateUseDateFormat} 日期格式化输出，日期格式为：yyyy-MM-dd HH:mm:ss</li>
     * <li>{@link com.alibaba.fastjson.serializer.SerializerFeature#BrowserCompatible} 解决中文乱码，会转成Unicode码</li> </ul>
